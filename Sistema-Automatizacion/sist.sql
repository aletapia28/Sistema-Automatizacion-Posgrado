...
CREATE DEFINER=`root`@`localhost` PROCEDURE `EditarMensajeria` (IN `newcorreo` VARCHAR(50), IN `newpassword` VARCHAR(50))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `EditarSuperusuario` (`newcorreo` VARCHAR(50), `newpassword` VARCHAR(100), `newcorreoEnvio` VARCHAR(50))  BEGIN
	START TRANSACTION;
    UPDATE usuarios SET password = newpassword WHERE correo = newcorreo;
...
('gpm@itcr.ac.cr', '4e87d307dde6d084c2b39f5f4ca41a20');
